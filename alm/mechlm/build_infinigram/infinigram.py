...
class InfiniGramModel:
    ...
    def from_data(
            ...
            documents_tkn: Union[list[int], np.array]=None,
        ...
        assert nworkers > 0
        assert documents is not None or documents_tkn is not None
        
        if documents_tkn is None:
        ...
        elif isinstance(documents_tkn, list):
            documents_tkn = np.array(documents_tkn)
    ...
    def get_longest_matching_next(
        ...
        matching, _ = get_retrieved_substrings(
            matches_indices[0],
            matches_indices[1],
            self.suffix_array,
            self.documents_tkn,
    ...
    def predict_prob(
        ...
        if len(matching) > 0:
            matching_array = np.stack(matching, axis=0)
            index = matching_array[:, -1]
            data = np.ones(len(index), dtype=np.float32)
            # data = np.exp(-np.array(distances) / len(self.documents_tkn) * 2)
            # data = np.exp(-np.argsort(distances))
            result_count = coo_array((data, (index, [0] * len(index))), shape=(vocab_size, 1))
            sparse = (len(result_count.nonzero()[0]) == 1)
            result_count = result_count.toarray()[:, 0]

            if smoothing_type == 'add-one':
                result = (result_count + 1) / (result_count.sum() + vocab_size)
            else:
                result = (result_count / result_count.sum())
        else:
            effective_n = 0
