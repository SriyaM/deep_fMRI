...
class RPESelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        r_q2 = q.transpose(0, 1).contiguous().view(T, B*self.n_head, C // self.n_head)
        ...
        weight2 = att.permute(2, 0, 1, 3).contiguous().view(T, B * self.n_head, T)
        weight2 = weight2 @ r_v2
        weight2 = weight2.transpose(0, 1).contiguous().view(B, self.n_head, T, C // self.n_head)

        y = weight1 + weight2 # B x nh x T x hs
        y = y.transpose(1, 2).contiguous() # B x T x nh x hs
        y = y.view(B, T, C)
...
class ModifiedCausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn_for_v = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        # output projection
        self.c_proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
        self.attn_dropout = nn.Dropout(config.dropout)
...
class ModifiedAttentionOnlyBlock(nn.Module):

    def __init__(self, config):
        super().__init__()
        self.ln_1 = LayerNorm(config.n_embd, bias=config.bias)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.attn = ModifiedCausalSelfAttention(config)
        
    def forward(self, x, xv):
        x = xv + self.attn(self.ln_1(x), self.ln_2(xv))
        return x
...
class ModifiedTransformerBlock(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.ln_1a = LayerNorm(config.n_embd, bias=config.bias)
        self.ln_1b = LayerNorm(config.n_embd, bias=config.bias)
        self.attn = ModifiedCausalSelfAttention(config)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config)
        
    def forward(self, x, xv):
        x = xv + self.attn(self.ln_1a(x), self.ln_1b(xv))
        x = x + self.mlp(self.ln_2(x))
        return x
...
class GPT(nn.Module):
    ...
    def __init__(self, config, allow_embed_update=False):
        super().__init__()
        assert config.vocab_size is not None
        assert config.block_size is not None
        self.config = config

        block = {'AttentionOnlyBlock': AttentionOnlyBlock,
                 'ModifiedAttentionOnlyBlock': ModifiedAttentionOnlyBlock,
                 'TransformerBlock': TransformerBlock,
                 'ModifiedTransformerBlock': ModifiedTransformerBlock,
        ...
        self.transformer.wte.weight.requires_grad = allow_embed_update
        if config.use_trained_pe and hasattr(self.transformer, 'wpe'):
            self.transformer.wpe.weight.requires_grad = allow_embed_update
    ...
    def forward(self, input_tokens, indices=None, indices2=None, targets=None, distance_targets=None, temperature=1.0, lamb_ce=0., lamb_mse=0., lamb_kl=0., lamb_kl_reverse=0.):
        ...
        if targets is not None or distance_targets is not None:
            ...
            if lamb_mse > 0:
                loss_mse = F.mse_loss(distance_all, distance_targets, reduction='mean')
                loss += lamb_mse * loss_mse
        else:
    ...
    def get_distance(self, input_tokens, indices1, indices2, temperature=1.0):
        logits, _, _ = self.forward(input_tokens)
    ...
    @classmethod
    def from_pretrained(cls, model_type, override_args=None, allow_embed_update=False, load_block=True):
        ...
        model = cls(config, allow_embed_update=allow_embed_update)
        sd = model.state_dict()
        ...
        if load_block:
            if config.attention_block == 'TransformerBlock':
                ...
                if not config.use_trained_pe:
                    # sd_keys_hf.remove('transformer.wpe.weight')
                    sd_keys_hf.remove('transformer.wpe.inv_freq')
                for k in sd_keys_hf:
                    if any(k.endswith(w) for w in transposed):
                        ...
                        with torch.no_grad():
                            sd[k].copy_(sd_hf[k])
            elif config.attention_block == 'ModifiedTransformerBlock':
                sd_keys_hf = sd_hf.keys()
                sd_keys_hf = [k for k in sd_keys_hf if not k.startswith('transformer.h.') or int(k.split('.')[2]) < config_args['n_layer']]
                sd_keys_hf = [k for k in sd_keys_hf if not k.startswith('lm_head')]
                sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.masked_bias')] # ignore these, just a buffer
                sd_keys_hf = [k for k in sd_keys_hf if not k.endswith('.attn.bias')] # same, just the mask (buffer)
                if not config.use_trained_pe:
                ...
                for k in sd_keys_hf:
                    if k.endswith('.c_attn.weight') or k.endswith('.c_attn.bias'):
                        with torch.no_grad():
                            sd[k].copy_(sd_hf[k].t()[:sd[k].size(0)])
                            sd[k.replace('.c_attn.', '.c_attn_for_v.')].copy_(sd_hf[k].t()[sd[k].size(0):])
                    elif k.endswith('.ln_1.weight') or k.endswith('.ln_1.bias'):
                        with torch.no_grad():
                            sd[k.replace('.ln_1.', '.ln_1a.')].copy_(sd_hf[k])
                            sd[k.replace('.ln_1.', '.ln_1b.')].copy_(sd_hf[k])
    ...
    def configure_optimizers(self, weight_decay, learning_rate, learning_rate_embed, betas, device_type):
        ...
        decay_params_embed = [p for n, p in param_dict.items() if p.dim() >= 2 and 'wte' in n]
        nodecay_params = [p for n, p in param_dict.items() if p.dim() < 2]
        optim_groups = [
            {'params': decay_params, 'weight_decay': weight_decay},
            {'params': decay_params_embed, 'weight_decay': weight_decay, 'lr': learning_rate_embed},
            {'params': nodecay_params, 'weight_decay': 0.0}
        ]
        num_decay_params = sum(p.numel() for p in decay_params) + sum(p.numel() for p in decay_params_embed)
